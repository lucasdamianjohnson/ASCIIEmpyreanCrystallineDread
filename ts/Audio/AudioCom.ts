...
export class AudioCom {
  ...
  jsHeader = { "Content-Type": "text/javascript" };
  audioHeader = { "Content-Type": "audio/mpeg" };
  dataHeader = { "Content-Type": "text/json" };

  constructor(public httpPort: number = 8080) {}
   ...
   requestPlay(
    ...
    if(this.count > 100000) {this.count = 0};
...
/*     const prom = new Promise((resolve) => {
  ...
  async _getHTML(path: string, res: any) {
    let index;
    console.log(path);
    if (path == "/start") {
      if (!this.connected) {
  ...
  async _getSong(path: string, res: any) {
    const song = await fs.readFile(`.${path}`);
    res.writeHead(200, this.audioHeader);
    res.end(song, "utf-8");
  }
  async _getData(path: string, res: any) {
    if (path == "/data/update") {
