...
exports.ScreenTransitions = void 0;
class ScreenTransitions {
    constructor(DS) {
        this.DS = DS;
    }
    async doScreenTransition(type, scene1, scene2, direction) {
        ...
        const scene1Data = scene1.screenStates["1"];
        const scene2Data = scene2.screenStates["1"];
        const newScene = this._sealSceneForTransition(direction, scene1Data, scene2Data);
        if (type == "level" || type == "battle") {
        ...
        if (type == "right-menu") {
            this.DS.dsCom.defineMessageStyle("Info", scene2.styleMap[scene2.rawText["1"].styleMap]);
        }
        if (type == "bottom-menu") {
            this.DS.dsCom.defineMessageStyle("Warning", scene2.styleMap[scene2.rawText["1"].styleMap]);
        }
        if (type == "bottom-right-menu") {
            this.DS.dsCom.defineMessageStyle("Error", scene2.styleMap[scene2.rawText["1"].styleMap]);
        }
    ...
    async _doSceneTransition(type, direction, newSceneString, newScene) {
        if (type == "level" || type == "battle") {
        ...
        if (type == "level") {
            this.DS.levelManager.setActiveScene(this.DS.levelManager.activeMeta.id, newScene.id);
        }
        if (type == "battle") {
            this.DS.battleSceneManager.setActiveScene(this.DS.battleSceneManager.activeMeta.id, newScene.id);
        }
        if (type == "right-menu") {
            this.DS.menuSceneManager.rightMenuDrawer.setActiveMenuScene(this.DS.menuSceneManager.activeMeta.id, newScene.id);
            this.DS.menuSceneManager.rightMenuDrawer.currentRow = 0;
        }
        if (type == "bottom-menu") {
            this.DS.menuSceneManager.bottomMenuDrawer.setActiveMenuScene(this.DS.menuSceneManager.activeMeta.id, newScene.id);
            this.DS.menuSceneManager.bottomMenuDrawer.currentRow = 0;
        }
        if (type == "bottom-right-menu") {
            this.DS.menuSceneManager.bottomRightMenuDrawer.setActiveMenuScene(this.DS.menuSceneManager.activeMeta.id, newScene.id);
    ...
    async _transitionLeft(type) {
        this.DS.engine.currentWorldCollumn = SCREENWIDTH + 2;
        for (let i = this.DS.engine.currentWorldCollumn; this.DS.engine.currentWorldCollumn > 0; i--) {
            this.DS.engine.currentWorldCollumn--;
            await this.DS.dsCom.asyncSleep(30);
        }
    }
    async _transitionRight(type) {
        this.DS.engine.currentWorldCollumn = 0;
    ...
    async _transitionUp(type) {
        if (type == "battle" || type == "level") {
            ...
            for (let i = this.DS.engine.currentWorldRow; i > 1; i--) {
                ...
                await this.DS.dsCom.asyncSleep(60);
            }
        }
        if (type == "right-menu") {
            this.DS.menuSceneManager.rightMenuDrawer.currentRow = RIGHTMENUHEIGHT - 2;
            for (let i = this.DS.menuSceneManager.rightMenuDrawer.currentRow; i > 1; i--) {
        ...
        if (type == "bottom-menu") {
            this.DS.menuSceneManager.bottomMenuDrawer.currentRow =
                BOTTOMMENUHEIGHT;
            for (let i = this.DS.menuSceneManager.bottomMenuDrawer.currentRow; i > 0; i--) {
                this.DS.menuSceneManager.bottomMenuDrawer.currentRow--;
                await this.DS.dsCom.asyncSleep(60);
            }
        }
        if (type == "bottom-right-menu") {
            ...
            for (let i = this.DS.menuSceneManager.bottomRightMenuDrawer.currentRow; i > 1; i--) {
                this.DS.menuSceneManager.bottomRightMenuDrawer.currentRow--;
                await this.DS.dsCom.asyncSleep(60);
            }
        }
    }
    async _transitionDown(type) {
        if (type == "battle" || type == "level") {
            this.DS.engine.currentWorldRow = 0;
    ...
    _sealSceneForTransition(direction, sceneArray1, sceneArray2) {
        let newScene = [];
        let index;
        switch (direction) {
            case "up":
    ...
    async _runWipe1(rows, cols) {
        for (let i = cols; i >= 0; i--) {
            let row = rows;
            let col = i;
            do {
                if (row >= 0 && col >= 1 && col < SCREENWIDTH + 1) {
                    this.DS.dsCom.showAt("*", { row: row, col: col });
                    this.DS.dsCom.sleep(1);
                }
                row--;
                col--;
            } while (row >= 2);
    ...
    async _doWipe2() {
        this.DS.engine.sceneTransitiong = true;
        await this._runWipe2(this.DS.dsCom.DIM.red("[0]"), SCREENHEIGHT + 2, SCREENWIDTH - 2);
        await this._runWipe2_2(this.DS.dsCom.red("[0]"), SCREENHEIGHT + 2, SCREENWIDTH - 2);
    ...
    async _runWipe2(text, rows, cols) {
        for (let i = cols; i > 1; i -= 3) {
            for (let j = 2; j < rows; j++) {
                this.DS.dsCom.showAt(text, { row: j, col: i });
                this.DS.dsCom.sleep(2);
            }
        }
    }
    async _runWipe2_2(text, rows, cols) {
...
exports.ScreenTransitions = ScreenTransitions;
