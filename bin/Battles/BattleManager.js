...
class BattleManager {
    constructor(DS) {
        ...
        this.currentFearForms = {};
        this.formMap = [];
        this.currentBattleSet = {};
    }
    _createBattleCursor() {
        this.battleCursor = (this.DS.entityManager.createNewEntity("battle", {}, `BattleCursorEntity`, "", 0, 0));
    }
    _reMapFormMap() {
        ...
        for (const fomrsCats of Object.keys(this.currentBattleSet)) {
            for (const forms of this.currentBattleSet[fomrsCats]) {
                for (let i = 0; i < forms.num; i++) {
                    const newForm = (Helper_1.cloneData(this.DS.fearFormManager.getFearForm(fomrsCats, forms.id)));
                    const entity = (this.DS.entityManager.createNewEntity("battle", newForm, `FearFormEntity`, "", currentCollumn, currentRow));
                    ...
                    if (currentCollumn + collumnSpacing >
    ...
    async enterBattle(battleSet) {
        ...
        this.DS.battleSceneManager.setActiveScene("transitions", "transition-frame");
        setTimeout(async () => {
            this.DS.menuSceneManager.setActiveSceneMeta("right-battle-menu");
            this.DS.menuSceneManager.rightMenuDrawer.setActiveMenuScene("right-battle-menu", "bump");
            await Promise.all([
                this.DS.screenTranistions.doScreenTransition("battle", this.DS.battleSceneManager.activeScene, this.DS.battleSceneManager.getScene("title-card"), "up"),
                this.DS.menuSceneManager.rightMenuDrawer.popOut(),
                this.DS.screenTranistions.doScreenTransition("bottom-menu", this.DS.menuSceneManager.bottomMenuDrawer.activeScene, this.DS.menuSceneManager
            ...
            setTimeout(async () => {
                ...
                await await Promise.all([
                    this.DS.screenTranistions.doScreenTransition("battle", this.DS.battleSceneManager.activeScene, this.DS.battleSceneManager.getScene("1"), "down"),
                    this.DS.screenTranistions.doScreenTransition("right-menu", this.DS.menuSceneManager.rightMenuDrawer.activeScene, this.DS.menuSceneManager.getScene("1"), "up"),
                    this.DS.menuSceneManager.bottomRightMenuDrawer.popDown(),
                ]);
                this.DS.screenTranistions.doScreenTransition("bottom-right-menu", this.DS.menuSceneManager.bottomRightMenuDrawer.activeScene, this.DS.menuSceneManager
